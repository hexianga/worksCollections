...
<body>
  ...
  <nav id="nav">
	  <ul>
		...
		<li><a href="study.php" class="first">学无止境</a></li>
  ...
  <div id="main">
    <div class="content">
	  ...
	  <div class="list">
	    <ul>
		<?php
		$sql="select * from study order by date desc";
		$result=$pdo->query($sql);
		while(($value=$result->fetch(1))!=false){
			echo <<<EOT
			<li>
			<div class="pic">
			  <a href="article.php?table=study&id={$value['id']}"><img src="img/pic1.jpg" width="175px" height="117px" /></a>
			</div>
			<div class="article">
			  <h3><a href="article.php?table=study&id={$value['id']}">{$value['title']}</a></h3>
			  <p><a href="article.php?table=study&id={$value['id']}">作者：{$value['author']}</a>
			  <span>摘录日期：{$value['date']}</span>
			  <span>分类：{$value['class']}</span></p>
			  <p class="main-content"><a href="article.php?table=study&id={$value['id']}">　　{$value['content']}</a></p>
			</div>
		  </li>
EOT;
		}
