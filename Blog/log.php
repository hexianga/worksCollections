...
<body>
  ...
  <nav id="nav">
	  <ul>
		...
		<li><a href="log.php" class="first">我的日志</a></li>
    ...
    <div class="content">
	  ...
	  <div class="list">
		...
		while(($value=$result->fetch(1))!=false){
			...
			<li>
		    <div class="pic">
			  <a href="article.php?table=log&id={$value['id']}"><img src="img/pic1.jpg" width="175px" height="117px" /></a>
			</div>
			<div class="article">
			  <h3><a href="article.php?table=log&id={$value['id']}">{$value['title']}</a></h3>
			  <p><a href="article.php?table=log&id={$value['id']}"><img src="img/head.png" width="33px" height="33px" />{$value['author']}</a>
			  <span>发表日期：{$value['date']}</span>
			  <p class="main-content"><a href="article.php?table=log&id={$value['id']}">　　{$value['content']}</a></p>
			</div>
		  </li>
EOT;
		}
